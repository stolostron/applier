...
build-bin: doc-help
	...
	@rm -rf bin
	@mkdir -p bin
	GOOS=darwin GOARCH=amd64 go build -ldflags="-s -w" -gcflags=-trimpath=x/y  -o bin/applier . && tar -czf bin/applier_darwin_amd64.tar.gz LICENSE -C bin/ applier
	GOOS=darwin GOARCH=arm64 go build -ldflags="-s -w" -gcflags=-trimpath=x/y  -o bin/applier . && tar -czf bin/applier_darwin_arm64.tar.gz LICENSE -C bin/ applier
	GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -gcflags=-trimpath=x/y  -o bin/applier . && tar -czf bin/applier_linux_amd64.tar.gz LICENSE -C bin/ applier 
	GOOS=linux GOARCH=arm64 go build -ldflags="-s -w" -gcflags=-trimpath=x/y  -o bin/applier . && tar -czf bin/applier_linux_arm64.tar.gz LICENSE -C bin/ applier 
	GOOS=linux GOARCH=ppc64le go build -ldflags="-s -w" -gcflags=-trimpath=x/y  -o bin/applier . && tar -czf bin/applier_linux_ppc64le.tar.gz LICENSE -C bin/ applier 
	GOOS=linux GOARCH=s390x go build -ldflags="-s -w" -gcflags=-trimpath=x/y  -o bin/applier . && tar -czf bin/applier_linux_s390x.tar.gz LICENSE -C bin/ applier
	GOOS=windows GOARCH=amd64 go build -ldflags="-s -w" -gcflags=-trimpath=x/y  -o bin/applier.exe . && zip -q bin/applier_windows_amd64.zip LICENSE -j bin/applier.exe
...
build-krew: krew-tools
	...
	KREW_ROOT=`mktemp -d` KREW_OS=linux KREW_ARCH=amd64 $$KREW install --manifest=applier.yaml && \
	KREW_ROOT=`mktemp -d` KREW_OS=linux KREW_ARCH=arm64 $$KREW install --manifest=applier.yaml && \
	KREW_ROOT=`mktemp -d` KREW_OS=windows KREW_ARCH=amd64 $$KREW install --manifest=applier.yaml;
...
doc-help:
	...
	go build -o docs/tools/applier docs/tools/applier.go && PATH=docs/tools applier && rm docs/tools/applier
...
manifests: controller-gen
	$(CONTROLLER_GEN) $(CRD_OPTIONS) rbac:roleName=manager-role webhook paths="./..." output:crd:artifacts:config=api/applier-cli/v1alpha1/crd
